...
def training_thread(device_idx, config):
    ...
    while True:
        ...
        model_params, train_params, model_path = queue.get()
        params = {"model_params": model_params, "train_params": train_params, "model_path": model_path}
        ...
        try:
            ...
            print("Beginning evaluating instance on {}... with id {} model_params={}, train_params={}".format(
            ...
            logs, best_result = model.train_eval(instance_dir, None, None, test_loader, **train_params, mode="test")
            ...
            print("Evaluating instance complete with result:", new_result)
...
def grid_search(config_path):
    ...
    for ce_path in config_eval:
        with open(os.path.join(GRID_ROOT, ce_path, "params.json"), 'r') as f:
            cur_ce = json.load(f)
            cur_model_path = os.path.join(GRID_ROOT, ce_path, "checkpoint.pytorch")
            combos.append((cur_ce["model_params"], cur_ce["train_params"], cur_model_path))
    global queue
    queue = Queue()
    for combo in combos:
        queue.put(combo)
